...
{
#if DEBUG
    std::cout << "[Bureaucrat signForm() is called]" << std::endl;
#endif
    if (f.getIsSigned()) {
        std::cout << _name << " couldn't sign "
            << f.getName() << " because, this form is already signed." << std::endl;
    }
    else {
        try {
            f.beSigned(*this);
...
{
#if DEBUG
    std::cout << "[Bureaucrat executeForm() is called]" << std::endl;
#endif
    try {
        form.execute(*this);
